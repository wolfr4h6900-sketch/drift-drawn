...
function renderProductPage(){
  const p=getProduct(new URLSearchParams(location.search).get("id"))||PRODUCTS[0],el=document.getElementById("productPage"),images=p.images||[p.image];
  el.innerHTML=`<div class="product-layout"><div class="product-gallery"><div class="product-large"><img id="productMainImage" src="${images[0]}" alt="${p.name}"></div>${images.length>1?`<div class="product-thumbs">${images.map((image,index)=>`<button class="product-thumb${index===0?" active":""}" onclick="selectProductImage('${image}',this)"><img src="${image}" alt="${p.name} view ${index+1}"></button>`).join("")}</div>`:""}</div><div class="product-details"><p class="eyebrow">${p.category} • ${p.tag}</p><h1>${p.name}</h1><div class="price-big">${money(p.price)} <span class="old">${money(p.oldPrice)}</span></div><p class="description">${p.desc} Printed and packaged for a premium wall-display experience.</p><div class="qty"><button onclick="productQty(-1)">−</button><span id="productQty">1</span><button onclick="productQty(1)">+</button></div><button class="btn btn-red full" onclick="addToCart(${p.id},Number(document.getElementById('productQty').textContent))">Add to cart</button><div style="margin-top:18px;color:#777;font-size:11px;line-height:1.8">✓ Premium print finish<br>✓ Secure packaging<br>✓ Double-sided tape included</div></div></div>`;
}
function selectProductImage(image,button){document.getElementById("productMainImage").src=image;document.querySelectorAll(".product-thumb").forEach(thumb=>thumb.classList.remove("active"));button.classList.add("active")}
